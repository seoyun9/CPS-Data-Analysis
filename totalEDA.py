...
# matplotlib.use("Agg")  # 주석 처리: 화면에 시각화를 표시하려면 이 줄을 주석 처리하세요
...
def plot_type_distribution(label_col):
    ...
    plt.bar(
        x + width / 2,
        fail_counts / len(fail) if len(fail) > 0 else 0,
        width,
        label="failure share",
    )
    plt.xticks(x, cats)
...
def plot_RNF(bins=35):
    plot_failure_bundle(
        "RNF", ["Air temperature [K]", "Process temperature [K]"], bins=bins
    )
...
def check_label_alignment():
    ...
    mismatch = df_tmp[df_tmp["Any_subfailure"] != df_tmp["Machine failure"]][
        ["Machine failure", "Any_subfailure"]
    ]
